...
quantized for efficient local inference. The repository contains application code; the model should
be
...
* Data Management provides practical SQLite inspection, import, query, visualization, and schema
  tools.

## ✨ Features

* **Local LLM inference** through `llama.cpp` and a configurable GGUF model path.
* **Deferred dependency resolution** so optional local model and embedding dependencies do not
  prevent
...
* **Prompt Engineering mode** backed by the local SQLite `Prompts` table, including prompt
  generation,
...
9. Fall back to in-memory cosine similarity when vector-table support is unavailable and fallback is
   enabled.
